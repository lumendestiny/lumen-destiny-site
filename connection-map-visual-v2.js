(()=>{
  function ensureConnectionI18n(){
    ...
    script.src='/connection-map-i18n-v2.js?v=20260816-1';
    script.defer=true;
  ...
  const infer=text=>{
    ...
    for(const [group,words] of Object.entries(sets))if(words.some(word=>value.includes(word.toLowerCase())))return group;
    return 'other';
  };
  const numeric=text=>{const match=String(text||'').replace(/,/g,'').match(/[+-]?\d+(?:\.\d+)?/);return match?Number(match[0]):0};

  function removeLegacyArt(scope=document){
    scope.querySelectorAll('.node-animal,.node-icon,.center-animal-img,.center-animal,.selected-animal-img,.selected-animal').forEach(el=>el.remove());
  }
  ...
  function decorateRanking(){
    ...
    rows.forEach((row,index)=>{
      ...
      if(!meter){
        ...
        fill.className='rank-meter-fill';
      ...
      if(fill){
        fill.style.width=`${Math.max(8,Math.round(Math.abs(value)/max*100))}%`;
        fill.classList.toggle('down',value<0);
      }
  ...
  function decorate(){
    ...
    if(center){
      removeLegacyArt(center);
      center.setAttribute('aria-label',`${center.querySelector('strong')?.textContent||'나'}, 인연지도의 중심`);
    }

    if(selected&&!selected.hidden){
      removeLegacyArt(selected);
      selected.dataset.group=infer(selected.querySelector('.selected-head p')?.textContent||'');
    }
    decorateRanking();
  }
