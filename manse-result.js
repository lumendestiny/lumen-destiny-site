...
function getInput(){const params=new URLSearchParams(location.search);if(params.get('birthDate')){return {name:params.get('name')||'당신',birthDate:params.get('birthDate'),birthTime:params.get('birthTime')||'',calendarType:params.get('calendarType')||'solar',isLeapMonth:params.get('isLeapMonth')==='1',gender:params.get('gender')||'other'};}const raw=sessionStorage.getItem('lumenSajuInput');return raw?JSON.parse(raw):null;}
try{
 const input=getInput();if(!input)throw new Error('입력 정보가 없습니다. 무료 사주 입력 화면에서 다시 시작해주세요.');
 const [y,m,d]=input.birthDate.split('-').map(Number);let sy=y,sm=m,sd=d;
 if(input.calendarType==='lunar'){const converted=lunarToSolar(y,m,d,Boolean(input.isLeapMonth));sy=converted.solar.year;sm=converted.solar.month;sd=converted.solar.day;}
 let hour,minute=0;if(input.birthTime){[hour,minute]=input.birthTime.split(':').map(Number);}
 const saju=input.birthTime?calculateSaju(sy,sm,sd,hour,minute,{applyTimeCorrection:false}):calculateSaju(sy,sm,sd,12,0,{applyTimeCorrection:false});
 ...
 document.getElementById('resultIntro').textContent=`기준 양력 ${sy}년 ${sm}월 ${sd}일${input.birthTime?` · ${input.birthTime}`:' · 출생시간 미입력'}`;
 ...
 document.getElementById('timeCorrection').textContent=input.birthTime?'입력한 시각을 기준으로 시주를 계산했습니다.':'출생시간을 입력하지 않아 시주는 표시하지 않습니다.';
