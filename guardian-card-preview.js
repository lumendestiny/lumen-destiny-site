(()=>{
  ...
  const archiveGuardian=qs.get('fromArchive')==='1'?(qs.get('guardian')||''):'';
  function start(){
    ...
    const archiveNames={
      'fortune-cat':'행운냥이','koi':'비단잉어','sun-bird':'아기 봉황','new-deer':'새벽사슴','gold-hamster':'복다람',
      'moon-rabbit':'달토끼','dolphin':'소망돌고래','fire-fox':'불여우','leaf-turtle':'잎새거북','star-owl':'별부엉이',
      'nine-fox':'백호','sea-dragon':'청룡','unicorn':'주작','forest-turtle':'현무','wing-owl':'황금기린',
      'sky-dragon':'백룡','fire-phoenix':'주작','moon-tiger':'청호','qilin':'녹기린','black-turtle':'현무'
    };
    let guarding=false;
    function ensure(){
      ...
      if(!get('guardian-tier-art-style')){const s=document.createElement('style');s.id='guardian-tier-art-style';s.textContent=`
        ...
        .guardian-tier-art-shell{position:relative;width:100%;max-width:306px;margin:0 auto;border-radius:16px;isolation:isolate;overflow:hidden;display:flex;align-items:center;justify-content:center;background:transparent}
        .guardian-tier-art{display:block;width:100%;height:auto;max-height:472px;object-fit:contain;object-position:center;border-radius:15px;filter:drop-shadow(0 9px 16px rgba(24,28,38,.14));image-rendering:auto;backface-visibility:hidden}
        .guardian-tier-value{margin:5px 4px 0;text-align:center;color:#555d6a;font-size:.68rem;line-height:1.25;font-weight:750}
        .guardian-order-panel select,.guardian-order-shell select{height:48px!important;min-height:48px!important;line-height:normal!important;padding:0 42px 0 14px!important;overflow:visible!important;text-overflow:ellipsis!important;white-space:nowrap!important;font-size:15px!important;font-weight:700!important;box-sizing:border-box!important;background-position:right 13px center!important}
        .guardian-order-panel select:disabled{opacity:1;color:#111827;background:#f1f3f8;cursor:not-allowed}
        .guardian-tier-lock-note{display:block;margin-top:5px;color:#6b7280;font-size:.68rem;font-weight:700}
        ...
        .guardian-order-panel input{height:42px!important;min-height:42px!important;line-height:1.3!important;box-sizing:border-box!important}
        ...
        @media(max-width:900px){.guardian-tier-art-shell{max-width:380px}.guardian-tier-art{max-height:560px}.guardian-order-panel select,.guardian-order-shell select{font-size:15px!important}}
        @media(prefers-reduced-motion:reduce){.guardian-card-preview[data-tier="rare"] .guardian-tier-art-shell,.guardian-card-preview[data-tier="legendary"] .guardian-tier-art-shell:after{animation:none!important}}
      `;document.head.append(s)}
      return shell;
    }
    function archiveSrc(){
      if(!archiveGuardian||!archive?.items?.[archiveGuardian])return'';
      return (archive.basePath||'')+archive.items[archiveGuardian]+'?v='+encodeURIComponent(archive.version||'1');
    }
    function render(){
      const shell=ensure(),key=approved[tier.value]?tier.value:'basic',asset=approved[key],img=shell.querySelector('.guardian-tier-art');
      card.dataset.tier=key;card.dataset.assetPolicy=archiveGuardian?'archive-selected':asset.kind;guarding=true;
      ...
      const selectedSrc=archiveSrc()||asset.src;
      if(img.getAttribute('src')!==selectedSrc)img.setAttribute('src',selectedSrc);
      const selectedName=archiveGuardian?(archiveNames[archiveGuardian]||'선택 Guardian'):(tier.options[tier.selectedIndex]?.textContent||'Guardian');
      img.alt=selectedName+' 승인 HD 판매용 카드 이미지';
      const value=card.parentElement?.querySelector('.guardian-tier-value');
      if(value)value.textContent=archiveGuardian?`${selectedName} · ${labels[key]}`:labels[key];
      guarding=false;
    }
    const img=ensure().querySelector('.guardian-tier-art');
    new MutationObserver(()=>{if(guarding)return;const key=approved[tier.value]?tier.value:'basic',asset=approved[key],expected=archiveSrc()||asset.src;if(img.dataset.fallback!=='1'&&img.getAttribute('src')!==expected)render()}).observe(img,{attributes:true,attributeFilter:['src']});
