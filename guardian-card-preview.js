(()=>{
  ...
  function start(){
    ...
    const labels={
      basic:'Guardian Basic · 승인 판매용 이미지 · 100개 한정',
      custom:'Personal Wish · 승인 판매용 이미지 · 100개 한정',
      rare:'Rare Edition · 승인 판매용 이미지 · 5개 한정 · 변화하는 테두리',
      legendary:'Legendary Motion · 승인 HD 황금 용 · 1/1 · 라이브 모션'
    };
    let guarding=false;
    function ensure(){
      let shell=frame.querySelector('.guardian-tier-art-shell');
      if(!shell){
        shell=document.createElement('div');shell.className='guardian-tier-art-shell';
        const img=document.createElement('img');img.className='guardian-tier-art';img.decoding='async';img.loading='eager';
        const vector=document.createElement('div');vector.className='guardian-tier-vector';vector.hidden=true;
        shell.append(img,vector);frame.append(shell);
      }
      let value=card.parentElement?.querySelector('.guardian-tier-value');
      if(!value&&card.parentElement){value=document.createElement('div');value.className='guardian-tier-value';card.insertAdjacentElement('afterend',value)}
      if(!get('guardian-tier-art-style')){
        const s=document.createElement('style');s.id='guardian-tier-art-style';s.textContent=`
          ...
          .guardian-tier-art{display:block;width:100%;height:auto;max-height:560px;object-fit:contain;object-position:center;border-radius:18px;filter:drop-shadow(0 12px 22px rgba(24,28,38,.18));image-rendering:auto}
          .guardian-tier-vector{width:100%}.guardian-tier-vector svg{display:block;width:100%;height:auto;max-height:560px;border-radius:18px;filter:drop-shadow(0 12px 22px rgba(24,28,38,.18))}
    ...
    function loadLegendary(asset,cb){
      ...
      s=document.createElement('script');s.src=asset.rendererSrc;s.dataset.legendaryHdArt='true';s.onload=cb;s.onerror=()=>console.error('Failed to load approved Legendary artwork');document.head.appendChild(s)
    }
    function render(){
      const shell=ensure(),key=approved[tier.value]?tier.value:'basic',asset=approved[key];
      const img=shell.querySelector('.guardian-tier-art'),vector=shell.querySelector('.guardian-tier-vector');
      card.dataset.tier=key;card.dataset.assetPolicy=asset.kind;
      guarding=true;
      if(key==='legendary'){
        img.hidden=true;img.removeAttribute('src');vector.hidden=false;vector.replaceChildren();
        loadLegendary(asset,()=>{if(card.dataset.tier==='legendary')vector.innerHTML=window.__LUMEN_LEGENDARY_HD_ART__.legendary()});
      }else{
        vector.hidden=true;vector.replaceChildren();img.hidden=false;
    ...
    const shell=ensure(),img=shell.querySelector('.guardian-tier-art');
    new MutationObserver(()=>{
      if(guarding)return;
      const key=approved[tier.value]?tier.value:'basic',asset=approved[key];
      if(key!=='legendary'&&img.getAttribute('src')!==asset.src)render();
    }).observe(img,{attributes:true,attributeFilter:['src']});
    tier.addEventListener('change',render);wish?.addEventListener('change',render);name?.addEventListener('input',()=>{});render();
  }
  document.querySelectorAll('script[data-guardian-approved-manifest]').forEach(x=>x.remove());
  const s=document.createElement('script');s.src='/guardian-asset-manifest.js?v=guardian-approved-sales-lock-20260814-1';s.dataset.guardianApprovedManifest='true';s.onload=start;s.onerror=()=>console.error('Failed to load Guardian approved asset manifest');document.head.appendChild(s);
})();