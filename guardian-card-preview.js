(()=>{
  ...
  function start(){
    ...
    const labels={
      ...
      legendary:'Legendary Motion · 승인 고해상도 황금 용 · 1/1 · 라이브 모션'
    };
    let guarding=false;
    function ensure(){
      ...
      if(!get('guardian-tier-art-style')){const s=document.createElement('style');s.id='guardian-tier-art-style';s.textContent=`
        ...
        .guardian-tier-art-shell{position:relative;width:100%;max-width:390px;margin:0 auto;border-radius:18px;isolation:isolate;overflow:hidden}
    ...
    new MutationObserver(()=>{if(guarding)return;const key=approved[tier.value]?tier.value:'basic',asset=approved[key];if(img.getAttribute('src')!==asset.src)render()}).observe(img,{attributes:true,attributeFilter:['src']});
