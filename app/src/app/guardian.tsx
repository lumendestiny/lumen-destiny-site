...
import { fetchGuardianAvailability, GuardianAvailability, guardianOrderUrl } from '../lib/guardian-api';
...
export default function GuardianScreen(){
 ...
 const open=(p:GuardianProduct)=>{const a=availability[p.key];if(a?.soldOut)return;Linking.openURL(guardianOrderUrl(p.key,recommended||p.element))};
 return <ScrollView contentContainerStyle={s.page} refreshControl={<RefreshControl refreshing={loading} onRefresh={load}/>}>
  ...
  <Text style={s.foot}>발행수량은 웹과 동일한 서버 기준으로 확인합니다. 최종 주문 단계에서도 다시 품절 여부를 검증합니다.</Text>
 </ScrollView>
}
