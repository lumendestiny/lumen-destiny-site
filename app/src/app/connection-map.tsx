...
export default function ConnectionMapScreen(){
 ...
 const save=async()=>{if(!result)return;const member:NetworkMember={id:`${result.b.name}-${Date.now()}`,name:result.b.name,relation:other.relation.trim()||'지인',elements:result.b.elements,score:result.score,grade:result.grade,addedAt:new Date().toISOString()};setNetwork(await addConnectionMember(member));setSaveMessage(`${result.b.name}님을 인연지도에 추가했습니다. 출생정보 원본은 저장하지 않습니다.`)};
...
function NetworkMap({meName,meElements,members,compact}:{meName:string;meElements:Elements|null;members:NetworkMember[];compact:boolean}){
...
const styles=StyleSheet.create({
 ...
 personCard:{marginTop:12,minHeight:58,padding:12,borderRadius:14,backgroundColor:'#f8f8fb',flexDirection:'row',justifyContent:'space-between',alignItems:'center'},
 personTitle:{fontSize:16,fontWeight:'800'},
 personCopy:{marginTop:3,fontSize:13,color:'#666b77'},
 removeButton:{minWidth:44,minHeight:44,marginLeft:8,alignItems:'center',justifyContent:'center'},
 remove:{fontSize:12,fontWeight:'800',color:'#9a4747'},
