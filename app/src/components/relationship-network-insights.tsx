...
export default function RelationshipNetworkInsights({meElements,members}:Props){
 ...
 const summary=useMemo(()=>summarizeNetwork(meElements,members),[meElements,members]);
 const ranking=useMemo(()=>members.map(member=>({member,impact:summarizeMemberImpact(meElements,members,member.id)})).filter((item):item is RankedImpact=>!!item.impact).sort((a,b)=>b.impact.coverageDelta-a.impact.coverageDelta||b.member.score-a.member.score).slice(0,3),[meElements,members]);
 const selected=useMemo(()=>compareIds.map(id=>members.find(member=>member.id===id)).filter((member):member is NetworkMember=>!!member),[compareIds,members]);
 ...
 const compareCopy=a&&b?(winner?`현재 인연망에서 상대적으로 부족한 ${weakest} 기운을 기준으로 ${winner.name}님이 더 높은 비중을 가지고 있습니다.`:`두 사람의 ${weakest} 기운 비중은 같습니다.`):selected.length===1?'비교할 인연을 한 명 더 선택해 주세요.':'두 사람을 선택하면 현재 부족한 오행을 누가 더 많이 가지고 있는지 비교합니다.';
 if(!members.length)return null;
 return <View style={[styles.wrap,compact&&styles.wrapCompact]}>
  <Text style={styles.title}>인연 영향 분석</Text>
  <Text style={styles.lead}>사람이 늘어날수록 누가 인연망 균형에 어떤 영향을 주는지 비교해서 볼 수 있습니다.</Text>
  <Text style={styles.subTitle}>균형 기여 TOP 3</Text>
  ...
  {a&&b?<Text style={styles.disclaimer}>※ 이 비교는 현재 부족한 오행의 상대적 보유 비중을 보는 참고 지표이며 사람의 가치나 관계의 미래를 평가하지 않습니다.</Text>:null}
 </View>;
}
