...
const officialPath=path.join(root,'assets','flags','kr-official.svg');
...
const errors=[];
if(!fs.existsSync(officialPath)) errors.push('Missing canonical South Korea flag asset: assets/flags/kr-official.svg');
else if(fs.readFileSync(officialPath,'utf8').trim()!==expected.trim()) errors.push('Canonical South Korea flag asset was modified. Restore the approved kr-official.svg exactly.');
...
const textExt=new Set(['.js','.mjs','.css','.html','.json','.md','.txt']);
const skipDirs=new Set(['.git','node_modules']);
function walk(dir){for(const ent of fs.readdirSync(dir,{withFileTypes:true})){if(skipDirs.has(ent.name))continue;const p=path.join(dir,ent.name);if(ent.isDirectory())walk(p);else if(textExt.has(path.extname(ent.name))){const rel=path.relative(root,p).replaceAll('\\','/');if(rel==='scripts/flag-integrity-audit.mjs')continue;const t=fs.readFileSync(p,'utf8');if(t.includes('assets/flags/ko.svg')||t.includes('/assets/flags/ko.svg'))errors.push(`Legacy Korean flag reference found in ${rel}`);if(t.includes('flags-fix.css'))errors.push(`Legacy flag CSS reference found in ${rel}`);if(/\.lang-choice\[data-lang=["']ko["']\][^{]*\{[^}]*background(?:-image)?\s*:/s.test(t))errors.push(`CSS-drawn Korean flag is forbidden in ${rel}; use kr-official.svg only.`)}}}
walk(root);

const shell=fs.readFileSync(path.join(root,'service-shell.js'),'utf8');
if(!shell.includes('/assets/flags/kr-official.svg'))errors.push('service-shell.js must reference only /assets/flags/kr-official.svg for Korean.');
if(errors.length){console.error('\nFLAG INTEGRITY AUDIT FAILED\n'+errors.map(e=>' - '+e).join('\n'));process.exit(1)}
console.log('FLAG INTEGRITY AUDIT PASSED: official South Korea flag is locked and legacy variants are blocked.');
