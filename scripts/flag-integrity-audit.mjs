...
const officialRel='assets/flags/kr-official-locked-v2.svg';
...
const forbiddenPaths=[
  ...
  path.join(root,'assets','flags','kr-official-locked-v1.svg')
];
...
const flagTerms='flag|lang-choice|data-lang|kr-official|taegeuk';
const delayedFlagMutation=new RegExp(`(?:setTimeout|setInterval)\\s*\\([\\s\\S]{0,700}?(?:${flagTerms})[\\s\\S]{0,700}?(?:\\.src\\s*=|replaceChildren|innerHTML\\s*=|style\\.|setAttribute\\s*\\(\\s*["']src["'])`,'i');
const flagObserverMutation=new RegExp(`MutationObserver\\s*\\([\\s\\S]{0,900}?(?:${flagTerms})[\\s\\S]{0,900}?(?:\\.src\\s*=|replaceChildren|innerHTML\\s*=|style\\.|setAttribute\\s*\\(\\s*["']src["'])`,'i');
function walk(dir){for(const ent of fs.readdirSync(dir,{withFileTypes:true})){if(skipDirs.has(ent.name))continue;const p=path.join(dir,ent.name);if(ent.isDirectory())walk(p);else if(textExt.has(path.extname(ent.name))){const rel=path.relative(root,p).replaceAll('\\','/');if(rel==='scripts/flag-integrity-audit.mjs')continue;const t=fs.readFileSync(p,'utf8');if(/assets\/flags\/(ko|kr|korea|kr-official|kr-official-locked-v1)\.svg/.test(t))errors.push(`Legacy Korean flag reference found in ${rel}`);if(t.includes('flags-fix.css'))errors.push(`Legacy flag CSS reference found in ${rel}`);if(/\.lang-choice\[data-lang=["']ko["']\][^{]*\{[^}]*background(?:-image)?\s*:/s.test(t))errors.push(`CSS-drawn Korean flag is forbidden in ${rel}; use locked asset only.`);if(flagObserverMutation.test(t))errors.push(`Runtime Korean flag observer mutation is forbidden in ${rel}`);if(delayedFlagMutation.test(t))errors.push(`Delayed Korean flag mutation is forbidden in ${rel}`)}}}
walk(root);

const shell=fs.readFileSync(path.join(root,'service-shell.js'),'utf8');
if(!shell.includes('/assets/flags/kr-official-locked-v2.svg'))errors.push('service-shell.js must reference the locked Korean flag v2 asset.');
const lock=fs.readFileSync(path.join(root,'flag-runtime-lock.js'),'utf8');
if(/MutationObserver|setTimeout|setInterval|\.src\s*=|style\./.test(lock))errors.push('flag-runtime-lock.js must remain inert and must not mutate the Korean flag at runtime.');
if(errors.length){console.error('\nFLAG INTEGRITY AUDIT FAILED\n'+errors.map(e=>' - '+e).join('\n'));process.exit(1)}
console.log('FLAG INTEGRITY AUDIT PASSED: Korean flag is static, legacy variants are blocked, and runtime mutations are forbidden.');
