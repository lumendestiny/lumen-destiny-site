...
needReadOnly('privacy-record-map.js',privacyMap);
const d1Preflight=mustContain('functions/api/admin/d1-preflight.js',['LUMEN_INTERNAL_SECRET','unauthorized','readOnly:true','customerRowsReturned:false','D1 PREFLIGHT READY','paymentControl','sqlite_schema']);
needReadOnly('d1-preflight.js',d1Preflight);
if(/SELECT\s+[^;]*\b(display_name|giver_name|recipient_name|gift_message|story_text|shipping_name|shipping_phone|shipping_address1|shipping_address2|verification_token)\b[^;]*\bFROM\b/is.test(d1Preflight))throw new Error('d1-preflight.js must not select private customer values');
...
function needReadOnly(label,text){
  for(const sql of [/\bDELETE\s+FROM\b/i,/\bUPDATE\s+guardian_/i,/\bINSERT\s+INTO\b/i,/\.batch\s*\(/i]){
    if(sql.test(text))throw new Error(`${label} must remain read-only: matched ${sql}`);
  }
}

console.log('Security/privacy release audit passed: client secrets absent, admin/test guards present, privacy record map and D1 preflight are internal/read-only, privacy evidence wired, and internal surfaces protected from indexing/framing.');
