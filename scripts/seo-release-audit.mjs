...
for(const url of ['https://lumendestiny.com/','https://lumendestiny.com/compatibility/','https://lumendestiny.com/guardian/'])need('sitemap.xml',sitemap,new RegExp(`<loc>${url.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}</loc>`,'i'),`public URL ${url}`);
...
// Transactional/test surfaces must not be promoted as public discovery landing pages.
for(const file of ['guardian-order/index.html','guardian-gift/index.html']){
  if(!fs.existsSync(file))continue;
  const t=read(file);
  if(file.includes('guardian-gift')&&!/noindex/i.test(t))fail.push(`${file}: expected noindex`);
}

if(fail.length){console.error(fail.join('\n'));process.exit(1)}
console.log('SEO release audit passed: canonical/OG/Twitter/hreflang, robots and sitemap discovery boundaries are locked.');
