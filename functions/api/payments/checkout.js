...
export async function onRequestPost({request,env}){
  ...
  const id=clean(body?.guardianId,40).toUpperCase(),lang=clean(body?.lang||'ko',8),policyAccepted=body?.policyAccepted===true,policyVersion=clean(body?.policyVersion||POLICY_VERSION,80);
  ...
  if(open?.checkout_url)return json({ok:true,reused:true,checkoutId:open.checkout_id,guardianId:id,amount:Number(order.price_usd),currency:'USD',provider,checkoutUrl:open.checkout_url,expiresAt:open.expires_at||null,returnUrl:`${new URL(request.url).origin}/guardian-payment-result.html?id=${encodeURIComponent(id)}&lang=${encodeURIComponent(lang)}`,policyVersion:POLICY_VERSION,policyAcceptedAt:now});
  ...
  return json({ok:true,checkoutId,guardianId:id,amount:Number(order.price_usd),currency:'USD',provider,checkoutUrl:checkoutUrl.toString(),expiresAt,returnUrl,policyVersion:POLICY_VERSION,policyAcceptedAt:now});
}
