...
export async function onRequestPost({request,env}){
  ...
  let order;try{order=await env.GUARDIAN_DB.prepare(`SELECT id,tier,price_usd,display_name,payment_status,issuance_status FROM guardian_orders WHERE id=? LIMIT 1`).bind(id).first()}catch{return json({ok:false,error:'storage_read_failed'},500)}
  if(!order)return json({ok:false,error:'not_found'},404);
  if(order.payment_status==='paid'||order.issuance_status==='issued')return json({ok:false,error:'already_paid',verifyUrl:`/guardian-verify.html?id=${encodeURIComponent(id)}&lang=${encodeURIComponent(lang)}`},409);
  const origin=new URL(request.url).origin,checkoutId=makeCheckoutId(),createdAt=new Date().toISOString(),expiresAt=new Date(Date.now()+30*60*1000).toISOString(),returnUrl=`${origin}/payment-result.html?id=${encodeURIComponent(id)}&lang=${encodeURIComponent(lang)}`;
  ...
  let res,data={};try{res=await fetch(adapterUrl.toString(),{method:'POST',headers:{'content-type':'application/json','authorization':`Bearer ${env.LUMEN_PAYMENT_ADAPTER_SECRET}`},body:JSON.stringify({checkoutId,guardianId:id,amount:Number(order.price_usd),currency:'USD',description:`Lumen Guardian ${order.tier}`,customerLabel:clean(order.display_name,40),returnUrl,cancelUrl:returnUrl,metadata:{guardianId:id,tier:order.tier}})});try{data=await res.json()}catch{}}catch{await env.GUARDIAN_DB.prepare(`UPDATE guardian_checkout_sessions SET status='adapter_error',updated_at=? WHERE checkout_id=?`).bind(new Date().toISOString(),checkoutId).run().catch(()=>{});return json({ok:false,error:'payment_adapter_unreachable'},502)}
  const checkoutUrl=safeHttpsUrl(data?.checkoutUrl),providerSessionId=clean(data?.providerSessionId||data?.sessionId,160);
  if(!res.ok||!checkoutUrl){await env.GUARDIAN_DB.prepare(`UPDATE guardian_checkout_sessions SET status='adapter_rejected',updated_at=? WHERE checkout_id=?`).bind(new Date().toISOString(),checkoutId).run().catch(()=>{});return json({ok:false,error:'payment_adapter_error'},502)}
