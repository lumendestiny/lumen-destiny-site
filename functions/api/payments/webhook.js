...
export async function onRequestPost({request,env}){
  ...
  const event=clean(body?.event,40),id=clean(body?.guardianId,40).toUpperCase(),paymentReference=clean(body?.paymentReference,120);
  if(event!=='payment.succeeded')return json({ok:true,ignored:true,event:event||'unknown'});
  ...
  if(row.payment_status==='paid'&&row.issuance_status==='issued')return json({ok:true,id,status:'issued',idempotent:true});
  try{await env.GUARDIAN_DB.prepare(`UPDATE guardian_orders SET payment_status='paid',issuance_status='issued',payment_reference=?,paid_at=COALESCE(paid_at,?),issued_at=COALESCE(issued_at,?) WHERE id=?`).bind(paymentReference||null,now,now,id).run()}catch{return json({ok:false,error:'storage_write_failed'},500)}
